...
public class ARTemplateMenuManager : MonoBehaviour
{
    ...
    [SerializeField] [Tooltip("The screen space controller associated with the demo scene.")]
    ...
    void ShowMenu()
    {
        ...
        if (!m_ObjectMenuAnimator.GetBool("Show"))
        {
            ...
            // Native Method call
            // UnityPlayer 클래스 가져오기
            // MainActivity.Companion 객체 가져오기
            AndroidJavaClass mainActivityClass =
                new AndroidJavaClass("com.hoxy.hlivv.ui.unity.unity.NativeUnityBridge");
            AndroidJavaObject companionObject = mainActivityClass.GetStatic<AndroidJavaObject>("Companion");

            // MainActivity.Companion.getInstance() 호출하여 MainActivity의 인스턴스 가져오기
            AndroidJavaObject mainActivityInstance = companionObject.Call<AndroidJavaObject>("getInstance");

            // MainActivity의 pullUpDrawer 메서드 호출
            mainActivityInstance.Call("expendMenu");
